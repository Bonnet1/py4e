...
baseurl = "https://download.bls.gov/pub/time.series/jt/jt.data.1.AllItems"

cur.execute('''CREATE TABLE IF NOT EXISTS Dataset
    (id INTEGER PRIMARY KEY, , series_id TEXT, year INTEGER, period TEXT,
     value FLOAT, footnote_codes TEXT,
     survey_abbr TEXT, seasonal TEXT, industry TEXT, region_code TEXT,
     dataelement_code TEXT, ratelevel_code TEXT)''')
...
while True:
    if ( many < 1 ) :
        conn.commit()
        sval = input('How many datapoints:')
        if ( len(sval) < 1 ) : break
        many = int(sval)

    start = start + 1
    cur.execute('SELECT id FROM Dataset WHERE id=?', (start,) )
    try:
        row = cur.fetchone()
